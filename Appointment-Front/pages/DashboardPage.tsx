...
import { ScheduleConfig, DaySchedule, User } from '../types';
...
import { SparklesIcon, ArrowRightOnRectangleIcon, UserIcon, ChevronDownIcon, CalendarDaysIcon, LinkIcon, UsersIcon, ChatBubbleBottomCenterTextIcon, DocumentTextIcon } from '../components/icons';
...
type Tab = 'schedule' | 'bookingLink' | 'userManagement' | 'ratings';
...
const DashboardPage: React.FC = () => {
  ...
  switch (activeTab) {
    ...
    case 'bookingLink': return <BookingLinkPage />;
    case 'userManagement': return <UserManagementPage />;
    case 'ratings': return <RatingsPage />;
    case 'bookings': return <BookingsPage />;   // 👈 agregado
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
      <div className="lg:col-span-1 space-y-8 lg:sticky lg:top-28">
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
            <h2 className="text-xl font-bold text-gray-800 mb-4 border-b border-[var(--color-border)] pb-3">{t('dashboard.settings.title')}</h2>
            <div className="space-y-5">
                <div>
                    <label htmlFor="slotDuration" className="block text-sm font-medium text-gray-700">{t('dashboard.settings.duration')}</label>
                    <select
                        id="slotDuration"
                        name="slotDuration"
        ...
        <div className="bg-gradient-to-br from-orange-50 to-white p-6 rounded-2xl shadow-lg border border-orange-100">
          ...
          <div className="space-y-4">
              ...
              {!process.env.API_KEY && <p className="text-xs text-center text-yellow-700 bg-yellow-100 p-2 rounded-md">{t('error.gemini.apiKeyMissing')}</p>}
              {error && <p className="text-sm font-medium text-red-700 bg-red-100 p-3 rounded-md text-center">{error}</p>}
          </div>
        </div>
      </div>
      <div className="lg:col-span-2 space-y-8">
        <div className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100">
            <h2 className="text-2xl font-bold text-gray-800 mb-4 border-b border-[var(--color-border)] pb-3">{t('schedule.title')}</h2>
            <div className="space-y-4">
  ...
  const OwnerTabs = () => {
    const getTabClass = (tabName: Tab) => {
        const baseClass = "flex items-center gap-2 whitespace-nowrap border-b-2 py-4 px-1 text-sm font-medium transition-colors";
  ...
  return (
    <div className="min-h-screen bg-[var(--color-background)] text-[var(--color-text-primary)]">
       {isDocModalOpen && <DocumentationExportModal isOpen={isDocModalOpen} onClose={() => setIsDocModalOpen(false)} />}
                        ...
                        <div className="py-1" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                            {Object.entries(supportedLanguages).map(([langCode, langDetails]) => (
                                <button
                                    ...
                                    role="menuitem"
                                >
                                    <img src={langDetails.flagUrl} alt={`${langDetails.nativeName} flag`} className="w-5 h-auto rounded-sm border border-gray-200" />
