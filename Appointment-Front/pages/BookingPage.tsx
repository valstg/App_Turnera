...
import { ScheduleConfig, DaySchedule, DayOfWeek } from '../types';
import { DAYS_OF_WEEK } from '../constants';
import { useLanguage, supportedLanguages } from '../contexts/LanguageContext';
import { ClockIcon, CalendarDaysIcon, ChevronDownIcon, SparklesIcon } from '../components/icons';
    ...
    if (daySetting.isEnabled && daySetting.startTime && daySetting.endTime && config.slotDuration > 0) {
      ...
      while (currentTime < endTime) {
        ...
        const relevantRule = daySetting.overbookingRules
            .slice().sort((a, b) => b.startTime.localeCompare(a.startTime))
...
const BookingPage: React.FC = () => {
    ...
    const hasAnySlots = useMemo(() => Array.from(dailySlots.values()).some(slots => slots.length > 0), [dailySlots]);
    ...
    if (isLoading) {
        return <div className="min-h-screen bg-gray-100 flex justify-center items-center">{t('login.signingIn')}...</div>
    }
        ...
        return (
            <div className="min-h-screen bg-gray-100 flex justify-center items-center p-4">
                <div className="w-full max-w-md text-center bg-white p-8 rounded-2xl shadow-xl">
                    ...
                    <button onClick={resetBooking} className="mt-6 w-full px-4 py-2.5 font-semibold text-white bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] rounded-lg">
            ...
            <div className="min-h-screen bg-gray-100 flex justify-center items-center p-4">
                 <div className="w-full max-w-md">
                    <div className="bg-white rounded-2xl shadow-xl p-8">
                        <h1 className="text-2xl font-bold text-gray-900 text-center">{t('publicBooking.confirm.title')}</h1>
                        <div className="mt-4 bg-orange-50 text-center p-3 rounded-lg border border-orange-200">
                            <p className="font-semibold text-orange-800">{t('publicBooking.confirm.slot')} {t(`days.${selectedSlot.day}`)} @ {selectedSlot.time}</p>
                        </div>
                        <form onSubmit={handleBookingSubmit} className="mt-6 space-y-4">
                            {error && <p className="text-sm text-red-600 bg-red-50 p-3 rounded-md">{error}</p>}
                            <div>
                                <label htmlFor="name" className="text-sm font-medium text-gray-700">{t('publicBooking.confirm.yourName')}</label>
                                <input type="text" id="name" name="name" required className="mt-1 w-full p-2.5 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--color-primary)]"/>
                            </div>
                            <div>
                                <label htmlFor="email" className="text-sm font-medium text-gray-700">{t('publicBooking.confirm.yourEmail')}</label>
                                <input type="email" id="email" name="email" required className="mt-1 w-full p-2.5 bg-gray-50 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[var(--color-primary)]"/>
                            </div>
                            <div className="flex gap-3 pt-2">
                                <button type="button" onClick={() => setSelectedSlot(null)} className="w-full px-4 py-2.5 font-semibold bg-gray-200 hover:bg-gray-300 rounded-lg">
                                    {t('schedule.overbooking.cancel')}
                                </button>
                                <button type="submit" disabled={isSubmitting} className="w-full px-4 py-2.5 font-semibold text-white bg-[var(--color-primary)] hover:bg-[var(--color-primary-dark)] rounded-lg disabled:bg-gray-400">
